...
class AntHH():
    ...
    ************************+*********   Parámetros *************************************
    ...
    def __init__(self,instancia,resultado,ants,lobos,iterMax,run):
        ...
        linea = 'problem|solver|instance|population|run|iter|ds|metric|executionTime|ant_population'
        f.write(linea)
        f.write('\n')
        f.close();
    ...
    def Pheromona(self,t):
        ...
        self.Phe_x = (1 - self.evaporacion) * self.Phe_x + dTx
...
print("AntHH-GWO, configurando ...")

if len(sys.argv[1:]) != 7:
    print("Ingrese todos los argumentos: ruta_instancia,ruta_resultados,lobos_poblacion,iteraciones,corridas_start,corridas_end,ants_poblacion")
    ...
    if not sys.argv[7].isnumeric():
        ...
        for run in range(int(sys.argv[5]),int(sys.argv[6])+1):
            ...
            ACO = AntHH(sys.argv[1],sys.argv[2], int(sys.argv[7]), int(sys.argv[3]), int(sys.argv[4]),run)
